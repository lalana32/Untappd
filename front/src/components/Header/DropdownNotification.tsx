...
const DropdownNotification = () => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const response = await agent.Notifications.getUserNotifications(
          user?.id!,
        );
        console.log(response);
        setNotifications(response);

        const unreadNotifications = response.some(
          (notifications: Notification) => !notifications.isRead,
        );
        setNotifying(unreadNotifications);
      } catch (error) {
  ...
  const markAllAsRead = async () => {
    try {
      const response = await agent.Notifications.isRead(user?.id!);
          ...
          >
            ...
            <ul className="flex h-auto flex-col overflow-y-auto">
              {notifications.length === 0 ? (
                <li className="px-4.5 py-3">No notifications available.</li>
              ) : (
